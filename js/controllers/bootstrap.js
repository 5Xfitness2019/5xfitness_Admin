...
/* Controllers */
  ...
  app.controller('ModalInstanceCtrl', ['$scope', '$uibModalInstance', 'items', function($scope, $modalInstance, items) {
   ...
   $scope.stories_ok = function () {    
        
        if($scope.items.user_id=='')
        {
          $scope.validationerror ='User is required.';
        }else if($scope.items.description=='')
        {
          $scope.validationerror ='Description is required.';
        }else{
   ...
   $scope.service_type_ok = function () {    
        
        if($scope.items.service_type=='')
        {
          $scope.validationerror ='Service type is required.';
        }else{
          $modalInstance.close($scope.items);
        }
    ...
    $scope.saveCustomer = function(status, response) {
      ...
      if(response.error!=undefined){
        ...
        if($scope.items.subscription_id==undefined)
        {
          $scope.validationerror ='Subscription type is required.';
        }else{
          response.subscription_id = $scope.items.subscription_id;
   ...
   $scope.AddTimeOk = function () {    
        
        if($scope.day==undefined)
        {
          $scope.validationerror ='Day is required.';
        }else{
          var response = new Object();
           response.day = $scope.day;
           response.start_time = $scope.start_time;
           response.end_time = $scope.end_time;
   ...
   $scope.add_Subscription_ok = function () {    
        
        if($scope.items.subscription_id==undefined)
        {
          $scope.validationerror ='Subscription type is required.';
        }else{
          $modalInstance.close($scope.items.subscription_id);
        }
